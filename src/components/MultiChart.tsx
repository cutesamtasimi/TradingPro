...
export function MultiChart({ symbol }: MultiChartProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-gray-900">
      {/* Compact Header */}
      <div className="p-2 border-b border-gray-700 bg-gray-800 shadow-sm flex-shrink-0">
        <div className="flex items-center justify-between mb-1">
          <div>
            <h2 className="text-sm font-bold text-white">Multi-Timeframe Analysis</h2>
            <p className="text-xs text-gray-400">Compare {symbol} across timeframes</p>
          </div>
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-gray-300">Charts ({timeframes.length}/6)</span>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-1">
            {timeframes.map((timeframe, index) => {
              ...
              return (
                <div key={key} className="flex items-center justify-between bg-gray-700 rounded p-1">
                  <div className="flex items-center space-x-1 flex-1">
                    <span className="text-xs text-gray-400 min-w-0">#{index + 1}:</span>
                    <select
                      value={timeframe}
                      onChange={(e) => handleTimeframeChange(index, e.target.value as Timeframe)}
                      className="text-xs bg-gray-800 border border-gray-600 rounded px-1 py-0.5 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 flex-1 min-w-0 text-white"
                    >
                  ...
                  <div className="flex items-center space-x-1 ml-1">
                    ...
                    {timeframes.length > 1 && (
                      <button
                        ...
                        className="p-0.5 text-red-400 hover:bg-red-900 rounded transition-colors"
                        title="Remove chart"
                      >
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        <div className="p-2 space-y-3">
          {timeframes.map((timeframe, index) => {
            const key = `${timeframe}-${index}`;
            return (
              <div key={key} className="bg-gray-800 rounded-lg shadow-sm border border-gray-600">
                <div className="p-2 border-b border-gray-700 bg-gray-700 rounded-t-lg">
                  <div className="flex items-center justify-between">
                    <div>
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <select
                        value={timeframe}
                        onChange={(e) => handleTimeframeChange(index, e.target.value as Timeframe)}
                        className="text-xs bg-gray-800 border border-gray-600 rounded px-2 py-1 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-white"
                      >
                        {AVAILABLE_TIMEFRAMES.map(({ label, value }) => (
                          <option key={value} value={value}>{label}</option>
                        ))}
                  ...
                  {loading[key] ? (
                    <div className="h-full flex items-center justify-center">
                      <div className="text-center">
                        ...
                        <div className="text-gray-400">Loading {getTimeframeLabel(timeframe)} chart...</div>
                      </div>
                    </div>
                  ) : chartData[key] ? (
                    <Chart
